...
<html>
	<head>
		<title>Millennial School | Add Student</title>
	...
	<body>
		<?php include"navbar.php";?><br>
				...
				<div class="content">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							{
								$sq="insert into student(RNO,NAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SSEC,SIMG,AID) values('{$_POST["rno"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["AID"]}')";
					...
					<label> ID</label><br>
						<?php
							...
							if($res->num_rows>0)
							{
								...
								$no=substr($row1["ID"],1,strlen($row1["ID"]));
								$no++;
							}
			...
			<button type="submit" style="float:right;" class="btn" name="submit">Add</button>
				</div>
